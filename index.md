...
<style>
  ...
  .pub-links a[data-accent="true"] { color: var(--cyan) !important; }
  .logo-row {
...
<div class="social-links">
  <a href="https://www.linkedin.com/company/qfin-uva/" target="_blank" rel="noopener">
    <svg viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path d="M20.447 20.452h-3.554v-5.569c0-1.328-.027-3.037-1.852-3.037-1.853 0-2.136 1.445-2.136 2.939v5.667H9.351V9h3.414v1.561h.046c.477-.9 1.637-1.85 3.37-1.85 3.601 0 4.267 2.37 4.267 5.455v6.286zM5.337 7.433c-1.144 0-2.063-.926-2.063-2.065 0-1.138.92-2.063 2.063-2.063 1.14 0 2.064.925 2.064 2.063 0 1.139-.925 2.065-2.064 2.065zm1.782 13.019H3.555V9h3.564v11.452zM22.225 0H1.771C.792 0 0 .774 0 1.729v20.542C0 23.227.792 24 1.771 24h20.451C23.2 24 24 23.227 24 22.271V1.729C24 .774 23.2 0 22.222 0h.003z"/></svg>
    LinkedIn
  </a>
  <a href="https://qfinformatics.slack.com/" target="_blank" rel="noopener">
  ...
  <a href="https://docs.google.com/forms/d/e/1FAIpQLScgq7ggljOVLVyLeNbCSD1TisPGPgw7IfzfCVVLvTuo0sI-VQ/viewform?usp=header" target="_blank" rel="noopener">
  ...
  <div class="qfin-section">
    ...
    {% if upcoming and upcoming.size > 0 %}
      {% for event in upcoming %}
        <div class="qfin-card event-hero">
          <h3 style="color:#fff; margin-top:0;">{{ event.title }}</h3>
          <div class="qfin-meta"><strong style="color:var(--cyan);">Date:</strong> {{ event.date | date: "%A, %d %B %Y" }}</div>
          {% if event.time %}<div class="qfin-meta"><strong style="color:var(--cyan);">Time:</strong> {{ event.time }}</div>{% endif %}
          <div class="qfin-meta">
            <strong style="color:var(--cyan);">Location:</strong>
            {% if event.location %}
              {{ event.location }}
            {% else %}
          ...
          <div class="qfin-meta" style="margin-top:0.3rem;"><strong style="color:var(--cyan);">Who?</strong> Students, staff, researchers, artists, activists — anyone curious about queer & feminist informatics.</div>
          {% if event.description %}<p class="qfin-desc">{{ event.description }}</p>{% endif %}

          <!-- Full schedule -->
          {% if event.schedule %}
            <h4 style="margin-top:1.25rem; margin-bottom:0.5rem; color:var(--cyan) !important;">Schedule</h4>
            {% for s in event.schedule %}
              <div class="schedule-item">
                <div>
                  <div class="schedule-time">{{ s.time }}</div>
                  <div class="schedule-location">{{ s.room }}</div>
                </div>
                <div>
...
<li><div class="pub-line"><span class="pub-authors">Sosto, M. &amp; Barron-Cedeno, A.</span> <span class="pub-year">2024</span>. <span class="pub-title">{QueerBench}: Quantifying Discrimination in Language Models Toward Queer Identities.</span> <span class="pub-venue">arXiv preprint arXiv:2406.12399</span>. <span class="pub-links"> <a href="https://doi.org/10.48550/arXiv.2406.12399" target="_blank" rel="noopener">DOI</a> <a href="https://arxiv.org/abs/2406.12399" target="_blank" rel="noopener">URL</a></span></div></li>
      ...
      <div class="qfin-grid">
        {% for m in site.data.members %}
          <div class="qfin-card qfin-member">
            ...
            {% endif %}
            <div>
              <h3 style="margin:0 0 0.2rem 0; color:#fff;">{{ m.name }}</h3>
  ...
  <p class="footer-note">
    ...
    <br><br>
    <a href="https://www.linkedin.com/company/qfin-uva/" target="_blank" rel="noopener" style="color:var(--grey);">LinkedIn</a> ·
    <a href="https://qfinformatics.slack.com/" target="_blank" rel="noopener" style="color:var(--grey);">Slack</a>
  </p>
